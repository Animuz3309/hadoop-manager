...
public enum Capability {
    ...
    /**
     * <ul>
     * <li>Bypass permission checks on operations that normally require the file system UID of the process to match the
     * UID of the file (e.g., chmod(2), utime(2)), excluding those operations covered by the {@link #DAC_OVERRIDE} and
     * {@link #DAC_READ_SEARCH}.
     ...
     * <li>Set the set-group-ID bit for a file whose GID does not match the file system or any of the supplementary GIDs
    ...
    IPC_OWNER,
    /**
     * Bypass permission checks for sending signals (see kill(2)). This includes use of the ioctl(2) KDSIGACCEPT
     * operation.
     */
    KILL,
    /**
     ...
     * <li>Use setsockopt(2) to set the following socket options: SO_DEBUG, SO_MARK, SO_PRIORITY (for a priority outside
     ...
     * <ul>
     * <li>grant or remove any capability in the caller's permitted capability set to or from any other process. (This
     * property of CAP_SETPCAP is not available when the kernel is configured to support file capabilities, since
    ...
    /**
     * <ul>
     * <li>Perform a range of system administration operations including: quotactl(2), mount(2), umount(2), swapon(2),
     * swapoff(2), sethostname(2), and setdomainname(2).
     * <li>Perform privileged syslog(2) operations (since Linux 2.6.37, CAP_SYSLOG should be used to permit such
     * operations).
     ...
     * <li>Use ioprio_set(2) to assign IOPRIO_CLASS_RT and (before Linux 2.6.25) IOPRIO_CLASS_IDLE I/O scheduling
     * classes.
     * <li>Forge UID when passing socket credentials.
     * <li>Exceed /proc/sys/fs/file-max, the system-wide limit on the number of open files, in system calls that open
     ...
     * <li>Employ the TIOCSTI ioctl(2) to insert characters into the input queue of a terminal other than the caller's
     ...
     * <li>View kernel addresses exposed via /proc and other interfaces when /proc/sys/kernel/kptr_restrict has the
     * value 1. (See the discussion of the kptr_restrict in proc(5).)
     * </ul>
     */
    SYSLOG,
    /**
     ...
     * <li>Set real-time scheduling policies for calling process, and set scheduling policies and priorities for
     ...
     * <li>Raise msg_qbytes limit for a System V message queue above the limit in /proc/sys/kernel/msgmnb (see msgop(2)
     * and msgctl(2)).
     * <li>Override the /proc/sys/fs/pipe-size-max limit when setting the capacity of a pipe using the F_SETPIPE_SZ
     * fcntl(2) command.
