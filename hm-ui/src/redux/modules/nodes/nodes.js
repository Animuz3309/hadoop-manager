...
export function create({name, cluster}) {
  return {
    ...
    promise: (client) => client.post(`/api/clusters/${cluster}/nodes/${name}`)
  };
}
...
export function remove({name, cluster}) {
  return {
    ...
    promise: (client) => client.del(`/api/clusters/${cluster}/nodes/${name}`)
  };
}
...
export function add(name, address) {
  return {
    ...
    promise: (client) => client.put(`/api/nodes/${name}`, {params: {address: address}})
  };
}
...
export function deleteNode(name, address) {
  return {
    ...
    promise: (client) => client.del(`/api/nodes/${name}`)
  };
}
